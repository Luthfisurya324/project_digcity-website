...
const OrganizationSettings: React.FC = () => {
  ...
  const addPosition = async () => {
    ...
    try {
      const created = await positionsAPI.create({ name: newPositionName })
      setPositions((prev) => [...prev, created].sort((a, b) => a.order_index - b.order_index))
      setNewPositionName('')
    } catch (e) {
      const msg = e instanceof Error ? e.message : 'Gagal menambah jabatan.'
      alert(msg)
    }
  }
  ...
  const removePosition = async (id: string) => {
    try {
      ...
      const msg = e instanceof Error ? e.message : 'Gagal menghapus jabatan.'
      alert(msg)
    }
  }

  const onDragStartPos = (e: React.DragEvent<HTMLDivElement>, id: string) => {
    e.dataTransfer.setData('text/plain', id)
  }

  const onDropPos = async (e: React.DragEvent<HTMLDivElement>, targetId: string) => {
    const draggedId = e.dataTransfer.getData('text/plain')
    if (!draggedId || draggedId === targetId) return
    const ordered = positions.map((p) => p.id)
    const fromIndex = ordered.indexOf(draggedId)
    const toIndex = ordered.indexOf(targetId)
    if (fromIndex < 0 || toIndex < 0) return
    ordered.splice(toIndex, 0, ordered.splice(fromIndex, 1)[0])
    await positionsAPI.reorder(ordered)
    const newPos = [...positions]
    newPos.sort((a, b) => ordered.indexOf(a.id) - ordered.indexOf(b.id))
    setPositions(newPos)
  }
    ...
    <div className="space-y-6">
      ...
      <div className="bg-white dark:bg-[#1E1E1E] rounded-xl p-6 border border-slate-200 dark:border-[#2A2A2A]">
        <div className="flex items-center justify-between mb-3">
          ...
          <div className="flex gap-2">
            <input value={newPositionName} onChange={(e) => setNewPositionName(e.target.value)} placeholder="Nama jabatan" className="px-3 py-2 border rounded-lg dark:bg-[#1A1A1A]" />
            <button onClick={addPosition} className="px-3 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 flex items-center gap-2"><Plus size={16} /> Tambah</button>
          </div>
        </div>
        <div className="space-y-2">
          {positions.map((p) => (
            <div key={p.id} draggable onDragStart={(e) => onDragStartPos(e, p.id)} onDragOver={(e) => e.preventDefault()} onDrop={(e) => onDropPos(e, p.id)} className="flex items-center justify-between p-3 rounded-lg border border-slate-100 dark:border-[#2A2A2A]">
              <div className="flex items-center gap-2">
                <ArrowUpDown size={16} className="text-slate-400" />
                <span className="text-sm font-medium">{p.name}</span>
              </div>
              <button onClick={() => removePosition(p.id)} className="text-rose-600 hover:bg-rose-50 rounded-lg p-2"><Trash2 size={16} /></button>
            </div>
          ))}
          {positions.length === 0 && (
